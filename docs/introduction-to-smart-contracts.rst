...
`you` (in "lu" to be exact, `1 you` is `10**18 lu`) which can be modified by sending transactions that
include YOU.
...
It can include binary data (which is called "payload") and YOU.
...
Contracts can call other contracts or send `you` to non-contract
accounts by the means of message calls. Message calls are similar
to transactions, in that they have a source, a target, data payload,
YOU, gas and return data. In fact, every transaction consists of
...
The only way to remove code from the blockchain is when a contract at that address performs the ``selfdestruct`` operation. The remaining `you` stored at that address is sent to a designated target and then the storage and code is removed from the state. Removing the contract in theory sounds like a good idea, but it is potentially dangerous, as if someone sends `you` to removed contracts, the `you` is forever lost.
...
If you want to deactivate your contracts, you should instead **disable** them by changing some internal state which causes all functions to revert. This makes it impossible to use the contract, as it returns `you` immediately.
