...
(seq
  ...
  ;; --------------------------------------------------------------------------
  ;; Will revert if sent any You. We use the macro immediately so as
  ;;   to abort if sent any You during contract deployment.
...
BOOST_AUTO_TEST_CASE(transfer_from)
{
	...
	// Send account(1) some you for gas.
	sendEther(account(1), 1000 * you);
	BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * you);
